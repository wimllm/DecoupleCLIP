...
from .DecoupleCLIP_clip_model import CLIP, convert_to_custom_text_state_dict, \
    resize_pos_embed
from .DecoupleCLIP_clip_model import build_model_from_openai_state_dict, convert_weights_to_lp, get_cast_dtype
...
_VITB16 = dict(
    ...
    last=_pcfg(
        "https://github.com/ChengShiest/LAST-ViT/releases/download/weights/openai_b_16.pt"
    )
)
...
def create_model(
    ...
    local_clip_path = Path(__file__).resolve().parent.parent / "open_clip111.bin"
    if local_clip_path.is_file():
        pretrained_source = str(local_clip_path)
        logging.info(f'Loading pretrained {model_name} from local checkpoint: {pretrained_source}.')
    else:
        pretrained_source = model_name
